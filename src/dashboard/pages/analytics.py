...
def show():
    st.title("Analytics")
    st.write("Anu00e1lise de dados histu00f3ricos de desastres e prediu00e7u00f5es do modelo de Machine Learning.")
    ...
    tab1, tab2, tab3 = st.tabs(["Dados Histu00f3ricos", "Prediu00e7u00f5es", "Modelo de ML"])
    ...
    with tab1:
        ...
        if df_disasters is not None and not df_disasters.empty:
            ...
            if all(col in df_disasters.columns for col in ['Disaster Type', 'Total Deaths', 'Total Affected']):
                # Agrupar por tipo de desastre
                disaster_summary = df_disasters.groupby('Disaster Type').agg({
                    'Total Deaths': 'sum',
                    'Total Affected': 'sum'                    
                }).reset_index()
                ...
                fig = px.bar(disaster_summary, x='Disaster Type', y=impact_metric,
                           title=f'{impact_metric} por Tipo de Desastre',
                           color='Disaster Type',
                           labels={'value': impact_metric, 'Disaster Type': 'Tipo de Desastre'})
                ...
                if 'Year' in df_disasters.columns:
                    ...
                    # Agrupar por ano
                    yearly_data = df_disasters.groupby('Year').size().reset_index(name='count')
                    ...
                    if 'Disaster Type' in df_disasters.columns:
                        yearly_by_type = df_disasters.groupby(['Year', 'Disaster Type']).size().reset_index(name='count')
                        
                        fig = px.line(yearly_by_type, x='Year', y='count', color='Disaster Type',
                                    title='Nu00famero de Desastres por Ano e Tipo',
                                    labels={'count': 'Nu00famero de Desastres', 'Year': 'Ano', 'Disaster Type': 'Tipo de Desastre'})
                        st.plotly_chart(fig, use_container_width=True)
            ...
            st.warning("Nu00e3o foi possu00edvel carregar dados histu00f3ricos de desastres.")
    ...
    with tab2:
        ...
        if predictions is not None and not predictions.empty:
            ...
            fig = px.bar(predictions, x='disaster_type', y='probability',
                        title='Probabilidade por Tipo de Desastre',
                        color='probability', color_continuous_scale='RdYlGn_r',
                        labels={'probability': 'Probabilidade', 'disaster_type': 'Tipo de Desastre'})
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    with tab3:
        ...
        feature_importance = pd.DataFrame({
            'Feature': ['temperature', 'humidity', 'pressure', 'water_level', 'soil_moisture', 'vibration', 'rain_level'],
            'Importance': [0.25, 0.18, 0.15, 0.22, 0.10, 0.05, 0.05]
        })
        
        fig = px.bar(feature_importance, x='Feature', y='Importance',
                    title='Importu00e2ncia das Caracteru00edsticas no Modelo',
                    color='Importance',
                    labels={'Importance': 'Importu00e2ncia', 'Feature': 'Caracteru00edstica'})
        st.plotly_chart(fig, use_container_width=True)
